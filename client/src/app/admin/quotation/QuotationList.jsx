...
const QuotationList = ({ searchQuery, statusFilter }) => {
  ...
  const { quotations, loading, error, successMessage } = useSelector(
  ...
  const handleApprove = () => {
    dispatch(
      approveQuotation({ id: selectedQuotation.id, status: "approved" }),
    );
    dispatch(getAllQuotations());
    closePopup();
  };
  ...
  const handleReject = () => {
    dispatch(
      approveQuotation({ id: selectedQuotation.id, status: "rejected" }),
    );
    dispatch(getAllQuotations());
    closePopup();
  };
                  ...
                  {currentRows.map((q, index) => (
                    <tr key={q.id} className="text-center hover:bg-gray-50">
                      ...
                      <td
                        ...
                        onClick={() =>
                          router.push(`/admin/quotation/invoice/${q.id}`)
                        }
                      >
                        {q.quote_no}
                      </td>
