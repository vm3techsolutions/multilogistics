...
import {
  fetchAllSeaQuotations,
  approveSeaQuotation,
  sendSeaQuotationEmail,
...
const SeaQuoteList = ({ searchQuery = "", statusFilter = "" }) => {
  ...
  const {
    ...
    loading,
    error,
  } = useSelector((state) => state.seaQuotation);
  ...
  /* ================= FETCH DATA ================= */
  useEffect(() => {
    dispatch(fetchAllSeaQuotations());
  ...
  /* ================= STATUS UPDATE ================= */
  const handleApprove = () => {
    dispatch(approveSeaQuotation(selectedQuotation.id)).then(() => {
      dispatch(fetchAllSeaQuotations());
      closePopup();
    });
  };
  ...
  const handleSendEmail = (id) => {
    dispatch(sendSeaQuotationEmail(id))
      .unwrap()
      .then(() => {
        alert("Sea quotation email sent successfully!");
        dispatch(fetchAllSeaQuotations());
      })
      .catch((err) => {
        alert(
  ...
  const currentRows = filteredQuotations.slice(
    indexOfFirstRow,
    indexOfLastRow
  );
  ...
  if (loading) return <p className="text-gray-500">Loading sea quotations...</p>;
                ...
                {currentRows.map((q, index) => (
                  <tr key={q.id} className="text-center hover:bg-gray-50">
                    ...
                    <td
                      ...
                      onClick={() =>
                        router.push(
                          `/admin/quotation/sea/invoice/${q.id}`
                        )
                      }
                    >
                      {q.quote_no}
